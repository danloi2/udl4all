...
interface UDLDataContextType {
  udlData: UDLData;
  udlIndex: UDLIndex;
  getConsiderationById: (id: string) => Consideration | undefined;
  getGuidelineForConsideration: (considerationId: string) => Guideline | undefined;
  getPrincipleForConsideration: (considerationId: string) => Principle | undefined;
  getExampleById: (id: string) => Example | undefined;
  getConsiderationForExample: (exampleId: string) => Consideration | undefined;
  getActivityById: (id: string) => Activity | undefined;
}

const UDLDataContext = createContext<UDLDataContextType | undefined>(undefined);

export function UDLDataProvider({ children }: { children: ReactNode }) {
  const udlData = rawData.udl;

  // Create indexed data for fast lookups
  const udlIndex = useMemo(() => {
    ...
    allActivityFiles.forEach((activityData: any) => {
      ...
      const activity: Activity = {
        ...
        duaAdaptations: Object.fromEntries(
          Object.entries(activityData.duaAdaptations as Record<string, any>).map(
            ([code, value]) => {
  ...
  // Helper functions (defined inside the provider to access udlData and udlIndex)
  const getConsiderationById = (id: string): Consideration | undefined => {
    return udlIndex.considerations.get(id);
  };

  const getGuidelineForConsideration = (considerationId: string): Guideline | undefined => {
  ...
  const getPrincipleForConsideration = (considerationId: string): Principle | undefined => {
    const guideline = getGuidelineForConsideration(considerationId);
    if (guideline) {
      for (const network of udlData.networks) {
        if (network.principle.guidelines.some((g: Guideline) => g.id === guideline.id)) {
  ...
  const getExampleById = (id: string): Example | undefined => {
    return udlIndex.examples.get(id);
  };

  const getConsiderationForExample = (exampleId: string): Consideration | undefined => {
    const parts = exampleId.split('-');
    if (parts.length >= 2) {
      const suffix = `${parts[parts.length - 2]}-${parts[parts.length - 1]}`;
      if (udlIndex.considerations.has(suffix)) {
        return udlIndex.considerations.get(suffix);
      }
    }
    return undefined;
  };
  ...
  const getActivityById = (id: string): Activity | undefined => {
    return udlIndex.activities.get(id);
  };

  return (
    <UDLDataContext.Provider
      value={{
        udlData,
        udlIndex,
...
export function useUDLData() {
  const context = useContext(UDLDataContext);
  if (context === undefined) {
    throw new Error('useUDLData must be used within a UDLDataProvider');
  }
  return context;
}
